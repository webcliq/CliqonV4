...
T.prototype = {
    ...
    _translate: function(){
        ...
        if(!src) return; 
		var bingkey = '6939EADE3AEB6916DB42C91B2491EFCE0770042D';	
		$.ajax({
			...
			crossDomain: true,
			//context: this,
			data: {appId: bingkey, from: _this.from, to: _this.to, contentType: "text/plain", text: src},
		...
		$.ajax({
			...
			success: function(languageCodes, status){
				$.ajax({
					...
					crossDomain: true,
					context: this,
					data: {appId: bingkey, locale: "en", languageCodes: '["'+languageCodes.join('", "')+'"]'},
