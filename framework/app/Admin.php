		 ...
		 **/
		 function miscellaneous($vars)
		 {
			...
			try {
			    ...
			    $rq = $vars['rq'];
				$idiom = $vars['idiom'];
				$table = 'dbcollection'; // Must be set
				
				// Javascript required by this method
				$js = "
			        Cliq.set('table', '".$table."');
			        Cliq.set('displaytype', 'miscellaneous');
			        Cliq.set('langcd', '".$idiom."');
			        Cliq.set('lcd', '".$idiom."');

			        $('#myTab a:first').tab('show');

			       	$.hook('misclink').on('click', function(e) {
	        			var btn = this;
	        			Cliqm.miscLink(e, btn);
	    			});
				";
			    $clq->set('js', $js);

				// Name of the Admin Component template which will be loaded from /admin/components/
				$tpl = "admmiscellaneous.tpl"; // This component uses Vue
